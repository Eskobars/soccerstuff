    ...
    for fixture_data in filtered_fixtures:
        ...
        if abs(home_team_rank - away_team_rank) >= 4:
            ...
            if rating in {'three_star', 'two_star', 'one_star'}:
                # Fetch injury and player data with rate limit handling

                ## Maybe we could make further limitations here by getting team data? Two api calls would happen before further calculation
                home_team_injuries, away_team_injuries = fetch_data_with_rate_limit(get_injury_data, fixture_id)
                home_team_players, away_team_players = fetch_data_with_rate_limit(get_player_data, fixture_id) ## This doesnt return much of anything, investigate if works on major leagues or not
